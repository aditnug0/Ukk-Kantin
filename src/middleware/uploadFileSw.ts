...
/** define storage configuration of image  */
const storage = multer.diskStorage({
    destination: (request: Request, file: Express.Multer.File, cb: (error: Error | null, destination: string) => void) => {
         ...
         * then create folder "image" inside of "public folder"
         */
        cb(null, `${BASE_URL}/public/siswa/`)
    },
...
const uploadFile = multer({
    storage,
    limits: { fileSize: 5 * 1024 * 1024 } /** define max size of uploaded file, in this case max size is 5 MB */
})
